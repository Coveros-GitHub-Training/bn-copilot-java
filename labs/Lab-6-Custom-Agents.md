...
- Use custom agents in different contexts (IDE, CLI, Cloud Agent)
...
- **Multi-Context**: Use agents in VS Code, CLI, or with Copilot Cloud Agent
...
6. **Name the file**: Enter `java-expert` (the `.agent.md` extension will be added automatically)

   This creates a file at `.github/agents/java-expert.agent.md`
...
Custom agents are available in VS Code, the Copilot CLI, and with Copilot Cloud Agent.
...
- [ ] Agent can be invoked via CLI or Cloud Agent
...
- Agents are usable in CLI, Cloud Agent, and soon in VS Code
